...
const EditCategory = ({close, fetchData, data:CategoryData}) => {
     ...
     const [data, setData] = useState({
      ...
      const handleOnChange = (e) => {
        ...
        setData((preve) => {
          return {
            ...preve,
            [name]: value, // Dynamically update the property with the name in 'name'
          };
        });
      };
    ...
    const handleUploadCategoryImage = async (e) => {
        ...
        setLoading(false) //uploading done setLoading false
        setData((preve) => {
          return {
            ...preve,
      ...
      <section className="fixed top-0 bottom-0 left-0 right-0 p-4 bg-neutral-800 bg-opacity-60 flex items-center justify-center">
           <div className="bg-white max-w-4xl w-full p-4 rounded">
             ...
             <form className="my-3 grid gap-2" onSubmit={handleSubmit}>
               ...
               <button className={
                 `${data.name && data.image ? "bg-green-500 hover:bg-green-600" : "bg-gray-300"}
                 px-4 py-2 font-semibold `}> Update Category
               </button>
             </form>
           </div>
      </section>
